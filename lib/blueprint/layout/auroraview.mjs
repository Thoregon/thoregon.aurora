...
import { forEach, Q }         from "/evolux.util/index.mjs";
...
export default class AuroraView extends View(AuroraBlueprintElement) {
    ...
    useParentViewModel() {
        return false; // !!this.getAttribute("set-parent-viewmodel") || !!this.getAttribute("embed") /*|| (await this.getViewModelClass()) == undefined*/;
    }
    ...
    async getViewModelProperty() {
        const property = this.modelProperty;
        if (!property || property === '.') return this.parentViewModel();
...
*/
    ...
    async existsConnect() {
        ...
        const q = this.renderQ;
        if (!q) return;
        this.renderQ = undefined;
        for (const fn of q) {
            fn();
        }
    }
    ...
    addRenderFn(fn) {
        if (!this.renderQ) return fn();
        this.renderQ.push(fn);
    }

...
export default class AuroraBlueprintElement extends AuroraElement {
    ...
    async applyContent(container) {
        ...
        try {
            ...
            // Todo: MN find all IDs  (ZOE)
            this.connectIDs(container);
        } finally {
            this._isRendering = false;
            if (this._renderWaitQ) {
                const queue = this._renderWaitQ;
                delete this._renderWaitQ;
    ...
    /*async*/ awaitRendering() {
        return new Promise((resolve) => {
            if (!this._isRendering) resolve();
            if (!this._renderWaitQ) this._renderWaitQ = [];
            this._renderWaitQ.push(resolve);
        })
    }
    ...
    getParentViewModel() {
        ...
        const parentViewModel = surroundingView?.viewModel;
    ...
    connectIDs( container ) {
        if ( this.id && this.id != '' ) { this.getAuroraBlueprint().registerID( this ); }
    }

 ...
 */
import { forEach }          from "/evolux.util/index.mjs";

import AuroraElement      from "../auroraelement.mjs";
import View               from "../blueprint/layout/view.mjs";
import MaterialCollection from "../../themes/material/collections/materialcollection.mjs";
...
export default class AuroraCollection extends AuroraElement {
    ...
    async mutated(mutation) {
        if (mutation.type === 'add')     await this.addItem(mutation.item, mutation.id);
        if (mutation.type === 'replace') await this.updateItem(mutation.item, mutation.id, mutation.oldItem);
        if (mutation.type === 'remove')  await this.removeItem(mutation.oldItem, mutation.id);
    }

    async updateItem(item, id, oldItem) {
        let elem = this.item4Update(id);
        elem.viewModel.model = item;
    }

    async removeItem(oldItem, id) {
        // let elem = this.item4Update(id);
        // elem.viewModel.model = item;
    }
    ...
    initQuery(query) {
        query.open();
        query.on(async (mutation) => await this.mutated(mutation));
/*

...
import MaterialPaypal  from "../../themes/material/paypal/materialpaypal.mjs";
...
export default class Paypal extends AuroraElement {
    ...
    static get observedAttributes() {
        return [...super.observedAttributes, 'active', 'payment-type', 'payment-method'];
    }

    behaviorClass() {
        return MaterialPaypal;
    }
...
*/
                ...
                'payment-type' : {
                    ...
                    description  : 'payment method',
                    group        : 'Spec',
                    example      : 'onetime | subscription | installment'
                },
                slotwidth  : {
                    default      : '100%',
                    type         : 'string',
...
*/
    ...
    set active(state) {
        this.setAttribute("active", state ? 'true' : 'false');
    }
    ...
    set paymentType(paymentType) {
        this.setAttribute("payment-type", paymentType);
        this.useType(paymentType);
    }
    ...
    start() {
        ...
        (async () => {
            ...
            if (this.isSubscription) {
                await this._renderSubscriptionButtons(handler, mountpoint);
            } else {
                await this._renderOrderButtons(handler, mountpoint);
            }
        })();
    }

    async _renderOrderButtons(handler, mountpoint) {
        window.paypal.Buttons({
            ...
            async createOrder(data, actions) {
                try {
                    ...
                    // tooo [OPEN]: display OK, disable paypal button ?
                    return order.id;
                } catch (error) {
    ...
    async _renderSubscriptionButtons(handler, mountpoint) {
        window.paypal.Buttons({
            fundingSource: paypal.FUNDING.PAYPAL, // Display only the PayPal button
            style        : {
                shape: 'rect',
            ...
            async createSubscription(data, actions) {
                try {
                    const { planId } = await handler.requestPaypalSubscription({});
            ...
            async onApprove(data, actions) {
                try {
                    const paypalId    = data.orderID;
                    const orderData   = await handler.requestPaypalApprove({ paypalId, data });
                } catch (error) {
                    handler.reportPaypalError(error);
                }
            },
    ...
    requestPaypalSubscription() {
        return new Promise((resolve, reject) => {
            console.log("-- Paypal Elem::requestPaypalOrder");
            const evt = { type: 'order', paypal: this };
            this._onSubscription = { resolve, reject };
            this.emit('order', evt);
        });
    }

    async subscriptionCreated(planId) {
        if (!this._onSubscription) {
            console.log("-- Paypal: create was not requested");
            return;
        }
        console.log("-- Paypal Elem::subscriptionCreated", planId);
        this._onSubscription.resolve({ planId });
    }

    async orderError(error) {
        if (this._onSubscription) return await this._onSubscription.reject(error);
    ...
    requestPaypalApprove(data) {
        return new Promise((resolve, reject) => {
            ...
            const evt = { type: 'approved', paypalId: data.paypalId, data: data.data, paypal: this };

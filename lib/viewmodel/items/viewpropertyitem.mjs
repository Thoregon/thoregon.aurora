...
export default class ViewPropertyItem {
    ...
    attachView(view) {
        ...
        propertyView.addEventListener('value', (evt) => this.valueChanged(evt.target.value));
        propertyView.addEventListener('input', (evt) => this.valueChanged(evt.target.value));   // for <text-area>
        propertyView.addEventListener('validate', (evt) => this.validate(evt));
        return this;
    }

...
export default class ViewPropertyItem {
    ...
    attachView(view) {
        ...
        if (!this.mutationHandler) {
            this.mutationHandler = (evt) => this.valueChanged(propertyView.value);
            this.validateHandler = (evt) => this.validate(evt, propertyView);
        }
        propertyView.addEventListener(eventname, this.mutationHandler);
        propertyView.addEventListener('validate', this.validateHandler);
        return this;
    }
    ...
    dispose() {
        const propertyView = this.vmitem.view;
        let eventname = propertyView.valueEventName || 'change';
        propertyView.removeEventListener(eventname, this.mutationHandler);
        propertyView.removeEventListener('validate', this.validateHandler);
    }
/*
    connect() {

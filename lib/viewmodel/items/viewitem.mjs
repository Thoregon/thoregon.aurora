...
export default class ViewItem extends VmEventEmitter(Object) {
    ...
    attachView(view) {
        ...
        if (view.isInput) {
            ...
            // its not an aurora element, but a html element with an 'aurora-name'
            if (view.auroraname) this.property = new ViewPropertyItem(this).attachView(view);
        }
        if (view.isTrigger) {
            // its not an aurora element, but a html element with an 'aurora-action'
            this.action = new ViewActionItem(this).attachView(view);
        } else {
            if (view.auroraaction) this.action = new ViewActionItem(this).attachView(view);
        }

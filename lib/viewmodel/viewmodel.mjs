...
import { propWithPath, elems, isString, callStack } from "/evolux.util/index.mjs";

import { Reporter }   from "/evolux.supervise/index.mjs";
import AccessObserver from "/evolux.universe/lib/accessobserver.mjs";
import { doAsync }    from "/evolux.universe/common.mjs";

const EXCLUDEPROPS = ['errors', '$thoregon', '$thoregonEntity', 'proxy$', '$entityResolver'];

const VM_PRIVATE_PROPERTIES = ['west', 'east', 'model', '@REF'];

const ALL_VIEWMODELS      = new Map();
const DISPOSED_VIEWMODELS = new Map();

export default class ViewModel extends Reporter(Object) {

    constructor(...args) {
        super(...args);
        this._handle            = 'VM_' + universe.random(5);
        this.mutationListeners$ = [];
        this.pathListeners$     = {model: [], meta: [], viewmodel: [], viewmeta: []};
        this.actions$           = [];

        // --- everything for translations
        this._translations = {};                        // Translation registry
        this._locale = navigator.language || 'en-EN';   // Requested locale
        this._resolvedLocale = null;                    // Resolved locale
        this._fallbackLocale = 'en-EN';                 // Fallback locale
        this._activeTranslations = {};                  // Cached active translations

        this.vmMutationListener = async (evt) => {
            if (evt.obj === universe.uirouter) return; // todo [OPEN]: just to avoid loops, find out why
            await doAsync();
            this.invokePathListeners('viewmodel', evt.property ?? evt.prop);
            this.invokeMutationListeners('viewmodel', evt.property ?? evt.prop);
        };

        this.modelMutationListener = async (evt) => {
            await doAsync();
            this.invokePathListeners('model', evt.property ?? evt.prop);
            this.invokeMutationListeners('model', evt.property ?? evt.prop);
        }

        const stack = callStack().splice(3);
        ALL_VIEWMODELS.set(this._handle, { vm: this, stack });
    }

    static get ViewModels() { return ALL_VIEWMODELS }
    static get DisposedViewModels() { return DISPOSED_VIEWMODELS }
...
*/
     ...
     setValueViewModelPath(path, value) {
        ...
        this.invokeMutationListeners('viewmodel', path);
    }
...
*/
            ...
            if (!ltype || ltype === type) {
                ...
                pathListeners.forEach(async (pathListener) => {
                    try {
                        ...
                        if (!lpath || lpath === '*' || lpath === path || path.endsWith(`.${lpath}`) || (isString(lpath) && lpath.endsWith('.*') && path.startsWith(lpath.slice(0,-1)))) {
    ...
    doFirstSync(type) {
        ...
        this.invokeMutationListeners(type);
    ...
    viewModelChange(evt) {
        ...
        this.invokeMutationListeners('viewmodel');
    }
    ...
    dispose() {
        this._disposed = true;
        const ref = ALL_VIEWMODELS.get(this.model);
        ALL_VIEWMODELS.delete(this._handle);
        if (ref) DISPOSED_VIEWMODELS.set(this._handle, { vm: new WeakRef(this), stack: ref.stack });
        this.eventDispose();
    }

    //
    // send widget events to outside docuemnt
    //
    ...
    redirectFromWidget(url) {
        this.emitWidgetEvent({ type: 'redirect', url });
    }
    ...
    /**
     * Register translations for the ViewModel.
     *
     * @param {Object} translations
     * Translation object grouped by locale.
     *
     * Example:
     * {
     *     'de-DE': { ... },
     ...
     *
     * @param {String} [options.fallbackLocale='en-EN']
     * Fallback locale used when no matching translation exists.
     *
     * Example:
     * fallbackLocale: 'en-EN'
     *
     * @param {String} [options.locale=navigator.language]
     ...
     * - manual language switchers
     * - admin preview modes
     * - testing
     *
     * @returns {ViewModel}
     */
    registerTranslations(translations, options = {}) {

        // Store translations
        this._translations = translations || {};

        // Get registered locales
        const registeredLocales = Object.keys(this._translations);

        // Set fallback locale
        this._fallbackLocale = options.fallbackLocale || this._fallbackLocale;

        // Ensure fallback locale exists
        if (!this._translations[this._fallbackLocale]) {
            this._fallbackLocale = registeredLocales[0] || null;
        }

        // Determine requested locale
        this._locale = options.locale || navigator.language || this._fallbackLocale;

        // Resolve active locale
        this._resolvedLocale = this.resolveLanguage(this._locale);

        // Cache active translations
        this._activeTranslations = this._translations[this._resolvedLocale] || {};

        return this;
    }

    /**
     * Add shared translations under a namespace.
     *
     * Existing VM translations are not overwritten.
     *
     * Example:
     * this.addTranslations('month', MonthTranslations);
     *
     * Result:
     * this.t('month.jan');
     *
     ...
     * @returns {ViewModel}
     */
    addTranslations(path, translations = {}) {
        ...
        Object.entries(translations).forEach(([locale, values]) => {
            ...
            if (!this._translations[locale][path]) {
                this._translations[locale][path] = {};
            }

            // Add only missing translations
            Object.entries(values).forEach(([key, value]) => {

                if (this._translations[locale][path][key] === undefined) {
                    this._translations[locale][path][key] = value;
                }
...
/*
        ...
        this._activeTranslations =
            this._translations[this._resolvedLocale] || {};

        // Reset cached params
        this._translationParams = null;
*/
        return this;
    }

    /**
     * Get translation interpolation parameters.
     *
     * Can be overwritten by subclasses
     * to provide dynamic translation values.
     *
     * Example:
     * {
     *     name: 'Martin',
     *     company: 'Summit-Suite'
     * }
     *
     * @returns {Object}
     */
    getTranslationParams() { return {}; }

    /**
     * Translate a path and optionally inject additional params.
     *
     * @param {String} path
     * Translation path.
     *
     ...
     *
     * @param {Object} [params={}]
     * Additional interpolation params.
     *
     * Example:
     * {
     *     name: 'Martin'
    ...
    t(path, params = {}) {
        ...
        const translationParams = {
            ...this.getResolvedTranslationParams(),
            ...params
        };

        return this.resolveTranslation(path, translationParams);
    }

    /**
     * Resolve a translation path and replace params.
     *
     * @param {String} path
     * Translation path.
     *
     ...
     * @returns {String}
     */
    resolveTranslation(path, params = {}) {

        const text = path
            .split('.')
            .reduce((object, key) => object?.[key], this._activeTranslations);
    ...
    getResolvedTranslationParams() {
        if (!this._translationParams) {
            this._translationParams = this.getTranslationParams();
        }
        return this._translationParams;
    }

    /**
     * Set the active locale externally.
     *
     * Useful for:
     * - language dropdowns
     * - user settings
    ...
    setLocale(locale) {
        ...
        // Store requested locale
        this._locale = locale || this._fallbackLocale;

        // Resolve active locale
        this._resolvedLocale =
            this.resolveLanguage(this._locale);
    ...
    resolveLanguage(locale) {
        ...
        // Use fallback locale if locale is missing
        if (!locale) {
        ...
        // Search similar locale
        const similarLocale = Object.keys(this._translations).find(
            key => key.startsWith(language + '-')
        );

        // Return similar locale if found
        if (similarLocale) {

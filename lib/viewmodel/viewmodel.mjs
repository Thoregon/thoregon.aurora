...
*/
    ...
    getWestProperties() {
        ...
        if (this.west && this.west.model) {
            ...
            JSINSPECTOR.getAllPropertyNames(model).filter((name) => !EXCLUDEPROPS.includes(name)).forEach((name) => props[name] = model[name]);
        }
        return props;
    }
    ...
    getVMProperties() {
        ...
        JSINSPECTOR.getAllPropertyNames(this).filter((name) => !EXCLUDEPROPS.includes(name)).forEach((name) => props[name] = this[name]);

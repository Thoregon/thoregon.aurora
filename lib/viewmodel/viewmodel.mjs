...
export default class ViewModel extends VmEventEmitter(Reporter(Object)) {
    ...
    static with({
        ...
        Object.assign(vm, { model, view, behavior });
        return vm;
    }
    ...
    toWest(mutation) {      // west = model
        ...
        if (mapping) {
            if (mapping.miditem) {
                if (!mapping.miditem.mutate(mutation)) {
                    if (mapping.modelitem) mapping.modelitem.mutate(mutation);
                }
            } else {
                if (mapping.modelitem) mapping.modelitem.mutate(mutation);
            }

...
import MaterialMultiSelect                               from "../../themes/material/form-multiselect/materialmultiselect.mjs";

export default class AuroraMultiSelect extends AuroraFormElement {

    behaviorClass() {
        return MaterialMultiSelect;
    }
    ...
    propertiesDefinitions() {
        ...
        return Object.assign(parentPropertiesValues, {
            ...
            limit: {
                default:        '',
                type:           'string',
                description:    'a number which tells the system how many multilevel terms can be added',
                group:          'Functionality',
                example:        '3'
            },
            placeholder: {
...
//        this._optionDefinitionType = 'html';
        ...
        switch (this._optionDefinitionType) {
            ...
            case 'taxonomy' :
                await this.extractTaxonomyOptions();
                break;
            case 'html':
    ...
    async extractTaxonomyOptions() {
        const propertiesValues = this.propertiesValues();
        const taxonomySlug     = propertiesValues['taxonomy'];
        const taxonomy         = await app.current.home.getTaxonomy(taxonomySlug);

        const termsOptions    = await taxonomy.getTermsAsOptions();
        this.behavior.options = termsOptions;
    }

    async addNewTaxonomyTerm(term) {
        ...
        let termId = await taxonomy.getTermByString(term);

        if (! termId) {
            termId = await taxonomy.addTerm(term);
        }

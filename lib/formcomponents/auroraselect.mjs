    ...
    */
    ...
    async existsConnect() {
        ...
        if (options.is_empty) {
            ...
            if (functionOptions in vm)        options = await vm[functionOptions]();
        }
    ...
    extractOptions() {
        const options = {};
        [...this.querySelectorAll('option')].forEach(option => options[option.getAttribute('value')] = option.innerText );
        return options;
    }
...
//               "i18n:esyp-product-name.label|Ratenzahlung"

            let newOption = new Option( result.defaultText , key);

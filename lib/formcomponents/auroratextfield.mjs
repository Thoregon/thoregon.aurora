...
import MaterialTextField from "../../themes/material/form-textfield/materialtextfield.mjs";
...
export default class AuroraTextField extends AuroraFormElement {
    ...
    static get observedAttributes() {
        return [...super.observedAttributes, 'label', 'placeholder', 'hint', 'errormsg', 'leading_icon', 'trailing_icon', 'disabled', 'readonly' ];
    }

    static forwardedAttributes() {
        return Object.assign(super.forwardedAttributes(), {
            placeholder: {select: 'input'},
            disabled   : {select: '.aurora-text-field, input'},
            readonly   : {select: '.aurora-text-field, input'}
        });
    }
    ...
    attributeChangedCallback(name, oldValue, newValue) {
        ...
        switch (name) {
            ...
            case 'hint':
                ...
                break;
            case 'errormsg':
                this.behavior.reportError(newValue);
            case 'leading_icon':
    ...
    propertiesDefinitions() {
        ...
        return Object.assign(parentPropertiesValues, {
            appearance: {
                default:        'q-outlined',
                type:           'string',
                description:    'Style of representation ',
                group:          'Styling',
                example:        'q-outlined | m3-outlined | m3 '
            },
            type: {
                default:        'text',
                type:           'string',
                ...
                group:          'Content',
                example:        'number | password | textarea | date '
            },
            label: {
                default    : '',
                type       : 'string',
            ...
            compactspacing: {
                default:        false,
                type:           'boolean',
                description:    'The field will use a more compace spacing between fields',
                group:          'Behavior',
                example:        ''
            },
            ...
            errormsg: {
                default    : '',
                type       : 'string',
                description: 'The error message which has to be shown',
                group      : 'Content',
                example    : 'Entered IBAN is incorrect',
                i18n       : true
            },
            elevation: {
                default:        '0',
    ...
    */
    collectClasses() {
        ...
        if ( propertiesValues['appearance'] ) { classes.push( propertiesValues['appearance']); }

        if ( propertiesValues['compactspacing'] ) { classes.push('compactspacing'); }
...
*/
    ...
    leadingIconClicked() {
        const event = new CustomEvent( "leadingiconclicked", {} );
        this.dispatchEvent(event);
    }
    trailingIconClicked() {
        const event = new CustomEvent( "trailingiconclicked", {} );
        this.dispatchEvent(event);
    }
    /*
     * Lifecyle

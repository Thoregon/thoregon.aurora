...
import MaterialButtonGroup from "../../themes/material/form-buttongroup/materialbuttongroup.mjs";
...
export default class AuroraButtonGroup extends AuroraFormElement {
    ...
    extractOptions() {
        ...
        [...this.querySelectorAll('option')].forEach(option => {
            ...
            let value = option.innerText?.trim();

            if (!value) {
                const i18n = option.getAttribute('aurora-i18n');

                if (i18n) {
                    value = {i18n: i18n};
                } else {
                    const bind = option.getAttribute('aurora-bind');
                    if (bind) {
                        value = {'aurora-bind': bind};
                    } else {
                        value = '*no default value or i18n*';
                    }
                }
            }
            options[option.getAttribute('value')] = value;
        });
    ...
    options(options) {
        ...
        const currentValue   = this.value ?? this._currentValue;
        group.innerHTML = '';
        ...
        for ( const [key, value] of Object.entries( options ) ) {
            ...
            let bind = '';
            if ( typeof value === 'object' && value !== null && 'aurora-bind' in value) {
                console.log('aurora-bind found:', value['aurora-bind']);
                bind = `aurora-bind="${value['aurora-bind']}"`;
                displayLabel = '';
            }

            let labelInnerHTML = `<i aria-hidden="true" class="material-icons">check</i><span ${bind}>${displayLabel}</span>`;
            ...
            if ( currentValue == key ) { input.setAttribute( 'checked', 'checked'); }
    ...
    get value() {
        if (!this.buttongroup) return this._currentValue;
    ...
    set value(value) {
        ...
        let radiobtn = this.buttongroup.querySelector('#'+ value);
        if (radiobtn) radiobtn.setAttribute('checked', '');
    }

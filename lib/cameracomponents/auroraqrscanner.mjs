...
export default class AuroraQRScanner extends AuroraCamElement {
    ...
    buildElement() {
        const root = this.builder.newDiv();
        root.id = "qrroot";
        return root;
    }
    ...
    async startScan() {
        ...
        try {
            ...
            this.dispatchEvent(new CustomEvent('qr-video-error', { detail: e.message }));
        }
    }

    scanResult(qrcode) {
        this.dispatchEvent(new CustomEvent('qr-code', { detail: qrcode }));
    }

    scanError(message) {
        this.dispatchEvent(new CustomEvent('qr-error', { detail: message }));
    }

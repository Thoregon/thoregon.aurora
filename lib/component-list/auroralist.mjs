    ...
    async renderTableData( query, rowfunction, columns, tablebody ) {
        ...
        for await (let key of query.itemKeys) {
            ...
            if ( itemviewmodelfunction ) {
                listItemViewModel = itemviewmodelfunction(object);
            } else {
                listItemViewModel = new ListItemViewModel();
            }

            listItemViewModel.model = object;
            ...
            //--- find all listitem actions
/*            let itemactions = tr.querySelectorAll('[aurora-listitem-action]');

            for (let i = 0; i < itemactions.length; i++) {
                const actionelement = itemactions[i];
                const action = itemactions[i].getAttribute('aurora-listitem-action');
                actionelement.addEventListener('click', ()=>this.callbackItemActionClicked( object, action ) );
            }*/
    ...
    renderActions(listItemViewModel, actionDefinition) {
        let object = listItemViewModel.model;
        //--- action-type:  button-list | button | container
        let display = '';
//        let actions = actionDefinition.actions

        let actions = object.metaClass.getAllActions()['all'];

        display += '<div class="aurora-table-actions">';
        display += '    <div class="aurora-table-actions-container"></div>';
...
//            if (isAvailable()) {
 //               if (is disabled) {  dddd }
  ...
  //          }
    ...
    renderAction(listItemViewModel, action, query) {
        let object = listItemViewModel.model;
        // todo: check if action is allowed for object

        return '<li class="aurora-table-action ' + action.name + '" aurora-action="'+ action.name +'"><span>' + action.label + '</span></li>';
    }

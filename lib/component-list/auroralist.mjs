...
import { deepdiff }        from "/evolux.util/lib/objutils.mjs"
import MaterialList        from "../../themes/material/component-list/materiallist.mjs";
...
export default class AuroraList extends AuroraFormElement {
    ...
    constructor() {
        ...
        this._tableEmptyContent = undefined;
    ...
    propertiesDefinitions() {
        ...
        return Object.assign(parentPropertiesValues, {
            'mode'          : {
                ...
                group      : 'Behavior',
                example    : 'table | grid | spreadsheet'
            },
            'title': {
                default    : '',
                type       : 'string',
            ...
            'attached' : {
                default    : '',
                type       : 'string',
                description: 'tells the system if the table is attached to another element',
                group      : 'Behavior',
            ...
            'rowclass': {
                default    : '',
                type       : 'string',
                description: 'defines the function responsible to deliver the class for the whole row',
                group      : 'Content',
                example    : 'error'
            },
            'itemviewmodel': {
            ...
            'empty-state-icon': {
                default    : 'note_stack',
                type       : 'string',
                description: 'Icon displayed in the empty state when no items are found',
                group      : 'Empty State',
                example    : 'note_stack'
            },
            'empty-state-title': {
                ...
                type       : 'string',
                description: 'Title text shown in the empty state when the list has no items',
                group      : 'Empty State',
            ...
            'empty-state-description': {
                default    : '',
                type       : 'string',
                description: 'Descriptive text providing context or guidance when the list is empty',
                group      : 'Empty State',
                example    : 'Try adjusting your search criteria or add your first product to get started.'
            },
            ...
            'empty-state-action-route' : {
                default    : '',
                type       : 'string',
                description: 'the route of the action',
                group      : 'Empty State',
                example    : '/product/new'
            },
            // view
    ...
    //--- attribute: list actions ---
    get listActions() {  return this._listActions;}
    set listActions(listActions) {
        this._listActions = listActions;

        this.determineAndSetHandler();
        this.handler.listActions = this.listActions;
        this.activateWhenReady();
    }
    ...
    set query( query ) {
        // source ...
        //  if (this._source  === query) return;

//        const listOfDiff = deepdiff(this._source, query);
//        if ( listOfDiff.length === 0  ) return;

//        this._source = deepdiff({}, query, { apply: true });
...
//                this._activated = false;
            ...
            this.determineAndSetHandler();
            const queryHasChanged = false; // (this.handler.query !== this.query);
            this.handler.query = this.query;
            this.activateWhenReady(queryHasChanged);
        }));
    }
    ...
    //--- table empty content ---
    get tableEmptyContent() { return this._tableEmptyContent }
    set tableEmptyContent(tableEmptyContent) {  this._tableEmptyContent = tableEmptyContent }
    ...
    activateWhenReady(queryHasChanged) {
        if ((queryHasChanged || !this._activated) && this.handler.isReady() ) {
...
*/
    ...
    applyTableDefinition() {
        ...
        const customStyle = this.querySelector('style')?.textContent || '';
    ...
    applyTableEmptyContent() {
        const emptyContent = this.querySelector('table-empty-content');
        if ( ! emptyContent ) {

            const propertiesValues = this.propertiesValues();

            const icon =  propertiesValues['empty-state-icon'];
            const title = propertiesValues['empty-state-title'];
            const description = propertiesValues['empty-state-description'];

            const actionLabel = propertiesValues['empty-state-action-label'];
            const actionIcon  = propertiesValues['empty-state-action-icon'];
            const actionRoute = propertiesValues['empty-state-action-route'];

            let html = '';
            html += '<div className="empty-wrapper">';

            html += icon  ? `<span class="material-icons empty-icon">${icon}</span>` : '';
            html += title ? `<div class="empty-title">${title}</div>` : '';
            html += description ? `<div class="empty-description">${description}</div>`: '';
            ...
            if ( actionRoute ) {
                html += `<br>
                    <aurora-button
                        ...
                        label="${actionLabel}"
                        icon="${actionIcon}"
                        aurora-route="${actionRoute}"
                    ></aurora-button>`;
            }

            html += '</div> <!-- end: empty-wrapper -->';

            this.tableEmptyContent = html;
        } else {
            this.tableEmptyContent = emptyContent.outerHTML;
        }
    }
...
*/
    ...
    applyTableActionDefinition() {

        const actionDefinition = this.querySelector('table-actions');
        if (!actionDefinition) return;

        const actionElements = [...actionDefinition.querySelectorAll('action')];
        const actionDefinitions = actionElements.map(action => (
            {
                type : action.getAttribute('type')  ?? 'button',
                id   : action.getAttribute('id')    ?? '',
                label: action.getAttribute('label') ?? '',
                style: action.getAttribute('style') ?? '',
                icon : action.getAttribute('icon')  ?? '',
            }
        ));
        if (actionDefinitions.length === 0) return;


        this.listActions = actionDefinitions;
    }
        ...
        const columnDefinitions = columnElements.map(columnElement => (
            {
                ...
                i18nlabel  : columnElement.getAttribute('i18nlabel') ?? '',
                ...
                editable   : columnElement.hasAttribute('editable'),
    ...
    updateSortSequence(column, order) {
        this.query.updateSortSequence(column, order);
    }
}
...
function  connectAuroraAttributes(element, viewModel) {
...
export class AuroraListHandler {
    ...
    constructor( element ) {
        ...
        this.rowClassFunction = this.properties['rowClass'];
...
//        };
    ...
    connectQuery( ) {
        this.query
            ...
            .onMove()   // implement
            .onSort(() => this.wasSorted())
            .onFlush(() => this.wasSorted());

    }

    wasSorted() {
        this.container.querySelector('.aurora-list-grid').innerHTML = '';
    }
...
export class AuroraListHandlerTable extends AuroraListHandler {
    ...
    transferProperties( element ) {
        ...
        this.propertiesValues = element.propertiesValues();
    }
    ...
    activate() {
        ...
        this.filter           = ''
        this.filterValue      = '';
        this.useRowClass      = false;
        this.rowClassFunction = '';

        const filter = auroralist.getAttribute('filter');
        if (filter) {
            this.useFilter = true;
            this.filter = filter;
            const filterValue = auroralist.getAttribute('filtervalue');
            if (filterValue) {
                this.filterValue = filterValue.toUpperCase();
            }
        }

        const vmfilter = auroralist.getAttribute('vmfilter');
        if (vmfilter) {
            this.useFilter = true;
            this.vmfilter = vmfilter;
        }

        const rowClass = auroralist.getAttribute('rowclass');
        if (rowClass) {
            this.useRowClass      = true;
            this.rowClassFunction = rowClass;
        }

        auroralist.applyTableDefinition();
        auroralist.applyTableEmptyContent();
        auroralist.applyFilterDefintion();
        auroralist.applyColumnDefinition();
        auroralist.applyTableActionDefinition();
        ...
        this._element.behavior.scaffoldedList();
    ...
    initializeElements() {
        this.__emptyContent      = this.element.container.querySelector('#table-empty-content');
    }
    ...
    scaffold() {
        ...
        if (this.propertiesValues.attached) {wrapperClasses.push('attached-' + this.propertiesValues.attached)};

        display += this.scaffoldCustomCSS();
        display += this.scaffoldActionHeader(wrapperClasses);
        display += this.scaffoldTableHeader(wrapperClasses);
        display += this.scaffoldTableBody();
        display += this.scaffoldTableFooter();

        display += this.scaffoldTableEmptyContent();

        const tableWrapper = this.container;
        tableWrapper.classList.add(...wrapperClasses);

        return display;
    }

    scaffoldCustomCSS() {
        const customCSS = this.element.tableDefinitions['customStyle'];
        if ( customCSS ) {
    ...
    scaffoldActionHeader(wrapperClasses) {

        const actions = this.listActions;

        const tableTitle = this.propertiesValues.title;

        if ( ! actions && ! tableTitle ) {
            return '';
        } else {
            wrapperClasses.push('feature-top-header');
        }

        let display = '';

        display += '<div class="table-top-header">';
        display += `<div class="table-title">${tableTitle}</div>`;
        if (actions && actions.length > 0) {
            display += '<div class="table-actions">';
            actions.forEach(action => {
                const icon = action.icon || '';
                display += `<div id="${action.id}" class="table-action ${action.style}">
                        <i class="material-icons table-action-icon">${icon}</i>
                        ${action.label}</div>`;
            });
            display += '</div> <!-- end: table-actions -->';
        }
        display += '</div> <!-- end: table-top-header -->';

        return display;
    }

    scaffoldTableHeader(wrapperClasses) {
        ...
        //--- table header -----------
        const hasLabel = columns.some(column => column.label !== "" || column.i18nlabel !== "");

        const theadClass = ( hasLabel ) ? '' : 'no-header';

        if ( hasLabel ) {
            wrapperClasses.push('feature-header');
        }

        display += `<thead class="noselect ${theadClass}">`;
        ...
        for (let c = 0; c < columns.length; c++) {
            ...
            const i18nLabel = (column.i18nlabel)
                ? `aurora-i18n:label="${column.i18nlabel}"`
                : '';

            display += /*column.header ? column.header :*/ `<div class="header-content" ${i18nLabel} >${column.label}</div>`;
    ...
    scaffoldTableEmptyContent() {
        this.tableIsEmpty = true;
        const emptyContent = this.element.tableEmptyContent;
        return `<div id="table-empty-content" class='table-empty-content'>${emptyContent}</div>`;
    }
    ...
    attachListActions() {
        ...
        actions.forEach(action => {
            const element = list.querySelector(`.table-action#${action.id}`);
            if (element) {
                element.addEventListener('click', () => {
                    const event = new CustomEvent(`${action.id}clicked`, {});
    ...
    async itemAdded(key, item, beforeKey) {
        ...
        const listItemBehavior = new AuroraListItemBehavior();
        ListItemViewModelClass = await this.getItemViewModelClass();
        listItemViewModel = await ListItemViewModelClass.with({ model: object, parent: this.getListViewModel(), behavior: listItemBehavior });
        ...
        if (this.useRowClass) {
            const rowClass = (this.useRowClass) ? await this.applyRowClass(listItemViewModel, object) : '';
            if (rowClass) {
        ...
        listItemBehavior.attachItem(tr, listItemViewModel);
        ...
        if (this.tableIsEmpty) {
            this.__emptyContent.style.display = "none";
    ...
    async insertItem( tr ) {
        ...
        tr.sortValue = itemToInsertSortValue;
        ...
        for (const listitem of items) {
            const itemSortValue = listitem.sortValue;
            if (itemSortValue > itemToInsertSortValue) {
                this.tablebody.insertBefore(tr, listitem);
                inserted = true;
                break;
            }
    ...
    async applyRowClass(listItemViewModel, object) {
        const functionName = this.rowClassFunction;
        // Todo: ask BL
        return await listItemViewModel[functionName](object);
    }
    ...
    async applyFilter(object) {
        ...
        if (this.useFilter) {
            if (this.vmfilter) {
                const vm = this._element.parentViewModel();
                if (!vm) return true;
                return await vm[this.vmfilter](object);
            }
    ...
    async renderTableRow( key, listItemViewModel ) {
        ...
        for (let c = 0; c < columns.length; c++) {
            ...
            if  ( column.classes )    { tdClasses.push(column.classes); }
            ...
            const edit = ( column.editable ) ? ' edit="' + column.attribute + '"' : '';

            displayColumn = '<td class="' + tdClasses.join(' ') + '" '+ edit +' ><div class="' + cwClasses.join(" ") + '" '+ cellRouting +'>' + displayColumn + '</div></td>';
...
//            display += '.aurora-grid-element { width: ' + this.properties['elementwidth'] + ' }';
    ...
    // todo: MN *itemviewmodel*
    async getItemViewModelClass() {
        if (this.itemViewModelClass) return this.itemViewModelClass;
        this.itemViewModelClass = (await this.element.getItemViewModelClass(this.element.propertiesValues()))  ?? ListItemViewModel;

        // todo: docu : this.itemViewModelClass = /*(await this.element.getItemViewModelClass(this.element.propertiesValues()))  ??*/ ListItemViewModel;
        return this.itemViewModelClass;
    }
 ...
 //       element.append(elementHTML);
    ...
    async insertItem( element ) {
        ...
        const itemToInsertSortValue = await itemToInsert[sort];
        element.sortValue = itemToInsertSortValue;
        ...
        for (const listitem of items) {
            const itemSortValue = listitem.sortValue;
            if (itemSortValue > itemToInsertSortValue) {
                this.grid.insertBefore(element, listitem);
                inserted = true;
                break;
            }
            // add listener for each item to resort on changes
    ...
    async clear() {
        ...
        const listGrid = this.grid;
        const elements = listGrid.querySelectorAll('.aurora-grid-element');
...
class AuroraListItemBehavior extends ThemeBehavior {
    attachItem(container, viewModel) {
        this.container = container;
        this.viewModel = viewModel;

        this.editableCells = container.querySelectorAll('td[edit]');
        this.editableCells.forEach((td, index) => {
            td.setAttribute('tabindex', '0');

            td.addEventListener('click', () => this.handleEditCell(td));
            td.addEventListener('keydown', (event) => this.handleKeyDown(event, index));
            td.addEventListener('focus', (event) => this.handleFocus(event, td));
            td.addEventListener('focusout', () => this.handleFocusOut(td));
        });
    ...
    handleEditCell(td) {
        ...
        const inputField = td.querySelector('input, select, textarea');
        if (!inputField) {
            const name  = td.getAttribute('edit');
            const input = document.createElement('input');
            input.type = 'text';
            input.setAttribute('aurora-name', name);

            td.appendChild(input);
            connectAuroraAttributes(td, this.viewModel);
    ...
    handleFocus(event, td) {
        // Handle focus when tabbing or clicking
        if (event.relatedTarget) {
            // This might be a placeholder to handle relatedTarget in complex scenarios
            console.log('Focus event related target:', event.relatedTarget);
        }
        console.log('Focused on:', td);
        this.handleEditCell(td);
    }
    ...
    handleKeyDown(event, currentIndex) {
        if (event.key === 'Tab') {
            ...
            let nextIndex;
            if (event.shiftKey) {
                nextIndex = (currentIndex - 1 + this.editableCells.length) % this.editableCells.length;
            } else {
                nextIndex = (currentIndex + 1) % this.editableCells.length;
            }

            this.handleFocusOut(this.editableCells[currentIndex]);
            this.handleEditCell(this.editableCells[nextIndex]);
        }
    }
}


class __AuroraListItemBehavior extends ThemeBehavior {
    attachItem(container) {
        this.container = container;

        const editableCells = container.querySelectorAll('td[edit]');

        // Iterate over each selected <td> element
        editableCells.forEach(td => {
            td.setAttribute('tabindex', '0'); // Makes the cell focusable

            // Add a click event listener that calls the editme function
            td.addEventListener('click',    ()      => this.handleEditCell(td));
            td.addEventListener('focusout', ()      => this.handleFocusOut(td));
            td.addEventListener('keydown',  (event) => this.handleKeyDown(event, editableCells, index));
    ...
    handleEditCell(td) {
        // Add the 'edit' class to the cell to indicate it's being edited
        td.classList.add('edit');
        // Focus the cell to enable focusout detection
        td.focus();
    }

    handleFocusOut(td) {
        // Remove the 'edit' class when the cell loses focus
        td.classList.remove('edit');
    }

    handleKeyDown(event, editableCells, currentIndex) {
        if (event.key === 'Tab') {
            event.preventDefault(); // Prevent the default tabbing behavior
            let nextIndex;
            if (event.shiftKey) {
                // If Shift+Tab is pressed, move to the previous cell
                nextIndex = (currentIndex - 1 + editableCells.length) % editableCells.length;
            } else {

...
export default class AuroraList extends AuroraFormElement {
    ...
    set query( query ) {
        ...
        this._query = Query.from(query);    // if it is a Query it won't get wrapped again!
    ...
    applyGridDefinition() {
        ...
        const gridTransitionsElements = [...gridTransitions.querySelectorAll('grid-transition')];
...
*/
    ...
    renderActions(listItemViewModel, actionDefinition ) {
        ...
        return `<aurora-actions mode="${actionDefinition.actionstyle}" position="${actionDefinition.align}" aurora-bind:actions="getActions('${actionDefinition.actionset}')">${view}</aurora-actions>`;
    }
...
export class AuroraListHandlerTable extends AuroraListHandler {
    ...
    async renderTableRow( listItemViewModel ) {
        ...
        for (let c = 0; c < columns.length; c++) {
            ...
            displayColumn = '<td class="' + tdClasses.join(' ') + '"><div class="cell-wrapper">' + displayColumn + '</div></td>';
...
//        this.registerVisibilityRules();
    ...
    scaffold(){
        ...
        const gridDimension = this.calculateGridDimensions( 6 );

...
import { propertywithpath }     from "/evolux.util/lib/pathutils.mjs";
...
// AuroraList
    ...
    callbackItemClicked(object, tr, route ) {
        ...
        for (let i = 0; i < ids.length; i++) {
            parameter[ids[i]] = propertywithpath(object, ids[i] );
        }
    ...
    renderTableRow(object, columns, query) {
        ...
        for (let c = 0; c < columns.length; c++) {
            ...
            if (column.type === 'actions') {
                ...
                if (column.route != '') {
                    display += '<a href="">' + this.getColumnValue(object, column ) + '</a>';
                } else {
                    display += this.getColumnValue(object, column );
    ...
    getColumnValue( object, column ) {
        return propertywithpath(object, column.attribute );
    }

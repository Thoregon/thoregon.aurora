...
// AuroraList
    ...
    async renderTableData( query, rowfunction, columns, tablebody ) {
        ...
        for await (let key of query.itemKeys) {
            ...
            // tr... get all element with attribute action-item-name addEventListener..
...
//        let actions = actionDefinition.actions
    ...
    renderAction(object, action, query) {
        ...
        return '<li class="aurora-table-action ' + action.name + '" aurora-item-action="'+ action.name +'"><span>' + action.label + '</span></li>';
    }

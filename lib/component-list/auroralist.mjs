...
// AuroraList
    ...
    async existsConnect() {
        ...
        switch ( propertiesValues['mode'] ) {
            case 'table':
                let handler = new AuroraListHandlerTable( this.container, vm, query, propertiesValues );
                handler.activate();
                break;
            case '_grid':
...
//               this.setResponsiveTableBreakpoints(columns);  // need to move
    ...
    connectQuery( ) {
     ...
     async itemAdded(key, item, beforeKey) {
        ...
        tr.addEventListener('click', () => this.callbackItemClicked(this.query, key, object, tr, this.route));

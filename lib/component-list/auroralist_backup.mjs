...
import VisibilityObserver from "../lib/visibilityobserver.mjs";
...
export default class AuroraList extends AuroraFormElement {
    ...
    query( query ) {
        ...
        switch ( propertiesValues['mode'] ) {
            case 'table':
                this._handler = new AuroraListHandlerList(this);
...
export class AuroraListHandlerList {
    ...
    activate () {
        this._element.container.innerHTML = this.scaffold();
        //this.connectQuery();
    }
    scaffold() {
 ...
 //       display += this.scaffoldTableBody();
    ...
    scaffoldTableHeader() {
        let columns = this._element._columns;
        let display = '';
        debugger;
        display += '<table class="">';

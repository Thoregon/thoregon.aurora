...
export default class AuroraValidationIndicator extends AuroraElement {
    ...
    async applyContent(container) {
        ...
        this.setAttribute('is-valid', true);
            // todo [RFACTOR]: redesign to pub/sub, don't scan document
        this.scanDocument();
    ...
    addAuroraChildNodes(collection, element) {
        ...
        if (element.auroraChildNodes) nodes = [...nodes, ...element.auroraChildNodes()];
        nodes.forEach(elem => {
    ...
    // todo [REFACTOR]: does not work anymore!
    //  - work with: aurora-bind:valid = "$.isValid"
/*

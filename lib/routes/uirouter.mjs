...
export default class UIRouter {
    ...
    async routePath0(route, wintarget = false, origin) {
        ...
        if (routematch) {
            if (wintarget) {
                const url = this.urlrouter.routeForHistory(route);
    ...
    async routePath(route, wintarget) {
        if (!route) return;
...
*/
    ...
    replaceEntityRouteNewWithRef(modelRef) {
        ...
        if (route === cur) return;
        runtime.history.replaceState({ current: route, title: route }, route, this.urlrouter.routeForHistory(route));
    }

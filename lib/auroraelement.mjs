...
import { missing }            from '/evolux.util/lib/objutils.mjs';
import { forEach, className } from '/evolux.util/lib/utilfns.mjs';
import Q                      from '/evolux.util/lib/requestqueue.mjs';
...
export default class AuroraElement extends UIElement {
    ...
    static get observedAttributes() {
        let forwardedAttrs = Reflect.ownKeys(this.forwardedAttributes());
    ...
    async processInitQ() {
        const initQ = this._initQ;
        if (initQ == undefined) return;
        delete this._initQ;
        for await (const fn of initQ) {
            await fn();
        }
    }
    ...
    triggerClicked(evt) {
        ...
        let elem = (evt.path?.[0] ?? evt.currentTarget ?? evt.target);
        let trigger;
        while(!trigger && elem) {
...
/*        if (this.elemHasFlex) {
    ...
    async addAppStyles(theme, styles) {
        ...
        // style for app
        if (!_appstyle[uiroot]) {
            const csspath = `${uiroot}/themes/${theme}/app/app.css`;
            try {
                let res = await this.fetch(csspath);
                if (res.ok) {
    ...
    async addColors(theme, styles) {
        ...
        if (!_colors[uiroot]) {
            try {
                let csspath = `${uiroot}/themes/${theme}/app/material.css`;
                let res = await this.fetch(csspath);
                if (res.ok) {
...
`
    ...
    propertiesValues() {
        ...
        for (let prop of Reflect.ownKeys( defaults ) ) {
 ...
 //                   propertiesValues[prop] = attributes[prop];
                ...
                propertiesValues[prop] = this[prop] ?? defaults[prop];
            }
    ...
    async themeBehavior(name) {
        const behaviorclass = this.behaviorClass();
        if (behaviorclass) return behaviorclass;
        ...
        return await Q(`${id}/behavior`, async () => {
            ...
            _behaviorpath[behaviorpath] = { id, name: this.constructor.name };
            return behaviormodule.default;
        });
    }

    static get behaviorregister() {
        return {
            modules: _behaviormodule,
            mapping: _behaviorpath,
        };
    }
    ...
    async fetch(path, opt) {
        if (app._viewTemplates?.hasOwnProperty(path)) return { ok: true, text: () => app._viewTemplates[path] };
        if (universe.AuroraTemplates?.hasOwnProperty(path)) return { ok: true, text: () => universe.AuroraTemplates[path] };
...
const nodepros = {
    ...
    getAuroraUIPath: {
        ...
        writable: true,
        value: function(sub) {
            let uipath = this.tagName ? this.tagName.toLowerCase() : '<??>';
            if (sub) uipath += `/${sub}`;
            if (this.uibase) return this.uibase.getAuroraUIPath(uipath);
            if (this.parentElement) return this.parentElement.getAuroraUIPath(uipath);
            return uipath;
        }
    },
    getAuroraBlueprint: {
        ...
        writable: true,
        value: function() {
            return universe.uirouter.getBlueprint();
...
Object.keys(nodepros).forEach((property) => {
    if (Node.prototype.hasOwnProperty(property)) delete nodepros[property];
})

Object.defineProperties(Node.prototype, nodepros);

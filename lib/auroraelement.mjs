 ...
 //                   propertiesValues[prop] = attributes[prop];
    ...
    async attachBehavior() {
        ...
        try {
            ...
            console.log(`Can't create behavior: '${this.elemId}' -> '${this.appliedTemplateName}'`);
        }
    }

    async themeBehavior(name) {
        if (_behaviormodule[this.elemId]) return _behaviormodule[this.elemId].default;
        return await Q(`${this.elemId}/behavior`, async () => {
            ...
            _behaviormodule[this.elemId] = behaviormodule;

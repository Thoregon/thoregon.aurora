...
export default class AuroraElement extends UIElement {
    ...
    applyContent(container) {
        this.applyTemplates(container);
        this.adjustContent(container);
        this.attachBehavior();
    }
    ...
    get templates() {
        ...
        tplnames.forEach((name) => templates[name] = this.uiComponents[theme][tplelem.component][`${theme}${name}`]);
        return templates;
    }
    ...
    elementStyle(templatename) {
        let tplelem = this.templateElements;
        if (!tplelem) return '';
        templatename = templatename ? `${tplelem.theme || 'material'}${templatename}` : `${tplelem.theme || 'material'}${this.appliedTemplateName}`
        let styles = [];
        let compcss = this.uiComponents[tplelem.theme || 'material'][tplelem.component][`$${tplelem.component}`];
        if (compcss) styles.push(compcss);
        let elemcss = this.uiComponents[tplelem.theme || 'material'][tplelem.component][`$${templatename}`];
        if (elemcss) styles.push(elemcss);
        return styles.join('\n');
    }
 ...
 //                   propertiesValues[prop] = attributes[prop];
    ...
    async themeBehavior() {
        ...
        if (!tplelem) return;
        let theme = tplelem.theme || 'material';
        let behaviorpath = this.uiComponents[theme][tplelem.component][`${theme}${this.behaviorname()}`];
        if (!behaviorpath) behaviorpath = `/thoregon.aurora/themes/${theme}/${tplelem.component}/${theme}${this.behaviorname()}`;

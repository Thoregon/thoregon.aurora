    ...
    static async dynamicElements(root) {
        ...
        await [...root.querySelectorAll('*')].aForEach(async (element) => {
            ...
            if (src) {
                ...
                try {
                    // check if there should is a collection if elements to import
                    if (Array.isArray(src)) {
                        await src.aForEach(async (item) => await import_(item));
                    } else {
                        await import_(src);
                    }
                } catch (e) {
                   if (thoregon.debug) console.log(`$$ can't import aurora-element ${tag}`, e);
                }
            }
        })
    }
    ...
    async getComponentConfig() {
        ...
        return await Q(`${this.elemId}/config`, async () => {
            ...
            try {
                ...
                if (thoregon.debug) console.log(e);
            }
            _config[this.elemId] = {};
        });
    }
 ...
 //                   propertiesValues[prop] = attributes[prop];
    ...
    async attachBehavior() {
        ...
        try {
            ...
            if (thoregon.debug) console.log(`Can't create behavior: '${this.elemId}' -> '${this.appliedTemplateName}': ${e.stack ? e.stack : e.message }`);
        }
    }
        ...
        return await Q(`${id}/behavior`, async () => {
            ...
            if (behaviorpath) {
                try {
                    ...
                    if (thoregon.debug) console.log(`Can't load behavior '${behaviorpath}': ${e.stack ? e.stack : e.message }`);

...
import { forEach, className, parseIni, joinPath, missing, Q, StateMachine } from '/evolux.util';
...
export default class AuroraElement extends UIElement {
    ...
    // aurora attribute attachment

    attachAuroraAttributes() {

    }

    attachInnerAuroraAttributes() {
    ...
    collectedProperties() {
        let properties = {
            ...
            //...this.getVMProperties(),
            ...this.globalProperties(), ...this.getCalculatedProperties()
        }
        return properties;
    }
    ...
    async applyTemplates(container) {
        ...
        // now 'aurora' attributes can be connected
        this.attachAuroraAttributes();
        this.attachInnerAuroraAttributes();
...
/*
        if (!_colors[appuiroot]) {
            try {
                let csspath = `${appuiroot}/views/blueprint/colors.xml`;
                let res = await this.fetch(csspath);
                if (res.ok) {
                    let xml         = await res.text();
                    let colorDefs   = asColorDefinitions(xml);
                    let colorCSS    =  AuroraColors.asColorCSS(colorDefs);
                    _colors[appuiroot] = colorCSS ? this.structCSS(colorCSS, csspath, 'Blueprint Colors') : '/!* no colors.xml *!/\n';
                    styles.push(_colors[appuiroot]);
                }
            } catch (ignore) { }
        } else {
            styles.push(_colors[appuiroot]);
...
*/
        ...
        //
        csspath = `${appuiroot}/blueprint/app.css`;
        try {
            let res = await this.fetch(csspath);
            if (res.ok) styles.push(this.structCSS(await res.text(), csspath, 'Blueprint'));
        } catch (ignore) { }
    }
    ...
    async addColors(theme, styles) {
        ...
        if (!_colors[uiroot]) {
            try {
                let csspath = `${uiroot}/${theme}/app/colors.xml`;
                let res = await this.fetch(csspath);
                if (res.ok) {
                    let xml         = await res.text();
                    let colorDefs   = asColorDefinitions(xml);
...
*/
    ...
    modelDisplayed() {
        let elements = this.container.querySelectorAll("[delayed-display]");
        elements.forEach(b=>b.removeAttribute('delayed-display'));
    }
    ...
    get auroravisible() {
        return this.getAttribute('aurora-visible');
    }

    get auroraenabled() {
        return this.getAttribute('aurora-enabled');
    }
    ...
    get isAuroraView() {
        return !!this.auroraname || !!this.auroraaction || !!this.auroravisible || !this.auroraenabled;
    }

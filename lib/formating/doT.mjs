...
function varname(code, c) {
  return !!code ? code.replace(`${c.argName}.`, '') : undefined;
}

function addVar(code, vars, c) {
  let name = varname(code, c);
  if (!name) return;
  vars.add(name.trim());
}
...
function template(tmpl, c, def) {
  ...
  str = (
    ...
    (c.strip
      ...
      .replace(/'|\\/g, "\\$&")
      .replace(syn.interpolate, (_, code) => {
        addVar(code, vars, c);

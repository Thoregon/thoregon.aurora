...
const REDEFINES = [...Object.keys(window).filter(name => name !== 'window' && name !== 'self' && name !== 'globalThis').filter(name => isNaN(name) && !AVAILABLES.includes(name))];
...
*/
    ...
    buildFN(expression, otherparams = []) {
        ...
        const fnparams = ['$scope', ...WRAPPED, ...CONTEXT_VARS, Object.keys(FUNCTIONS), ...otherparams];

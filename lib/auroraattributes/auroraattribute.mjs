...
export default class AuroraAttribute {
    ...
    static cleanListOfUnconnectedElements() {
        const unconnectedAuroraAttributes = [...AuroraAttribute.AuroraAttributes.values()].filter(a => !a.element.isConnected);
        if (unconnectedAuroraAttributes.length === 0) return;
        console.log("-- AuroraAttribute: found disconnected elements", unconnectedAuroraAttributes.length);
        unconnectedAuroraAttributes.forEach((auroraAttribute) => auroraAttribute.disconnect());
    }
}
...
setInterval(() => AuroraAttribute.cleanListOfUnconnectedElements(), 5000);

...
export default class AuroraAttributeI18N extends AuroraAttribute {
    ...
    resolveI18n() {
        ...
        replacements = i18nAttribute.slice( token.length )?.trim();
    ...
    parseReplacements(replacements) {
        const parts = replacements.slice(1, -1).split(',');
        const params = {};
        parts.forEach(part => {
            const p2 = part.split(':');
            if (p2.length < 2) return;
            const name = p2[0].trim();
            const fnbody = p2[1].trim();
            try {
                const ev = new Evaluator();
                const fn = ev.buildFN(fnbody);
                params[name] = fn;
            } catch (e) {
                console.log("aurora-i18n parse error", name, fnbody , e.message);
            }
        });
        return params;
    }

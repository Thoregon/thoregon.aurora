...
export default class AuroraAttributeIntersect extends AuroraAttribute {
    ...
    parseAttributeSelector() {
        ...
        name = parts.shift();
        if (parts.length < 1) return { name, selector: this.useRatioTrigger([0]) };
        if (parts[0].startsWith("enter") || parts[0].startsWith("leave")) return { name, selector: this.useStateTrigger(parts) };
        // if all parts are numbers use ratio trigger
        if (allNumbers(parts)) return { name, selector: this.useRatioTrigger(parts) };
        // report error

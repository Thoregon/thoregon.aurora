...
export default class AuroraAttributeIntersect extends AuroraAttribute {
    ...
    attachViewModel(viewModel, listViewModel) {
        super.attachViewModel(viewModel, listViewModel);
        this.parseAttributeTrigger();
    }

    parseAttributeSelector() {
        // todo [REFACTOR]: this is a damn workaround, get rid of it
    ...
    parseAttributeTrigger() {
        ...
        if (parts.length < 1) {
            ...
            // report error
            console.log(`aurora-intersect: '${this.attribute.value}' eval error >`, this.attribute?.name);
         }
    ...
    async elementTriggered($intersecting, $ratio) {
        ...
        const otherparams = { ...this.elementAndAttributeParams(), $intersecting, $ratio };
    ...
    get fn() {
        if (!this._fn) {
            try {
                ...
                const fn       = ev.buildFN(this.attribute.value, [...this.elementAndAttributeParamNamess(), '$intersecting', '$ratio']);
...
class Trigger {
    ...
    async elementTriggered(intersecting, ratio) {
        if (this.attribute.debounceId) {
            clearTimeout(this.attribute.debounceId);
        }
        this.attribute.debounceId = setTimeout(() => {
            delete this.attribute.debounceId;
            this.attribute.elementTriggered(intersecting, Math.round(ratio*100));
        }, INTERSECTDELAY);
    }

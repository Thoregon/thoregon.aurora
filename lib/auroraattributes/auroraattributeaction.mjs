...
export default class AuroraAttributeAction extends AuroraAttribute {
    ...
    static attrCompare() {
        ...
        return (attrname) => (attrname.startsWith(SHORTCODE) || names.findIndex(name => name === attrname || attrname.startsWith(name+':')) > -1);
    }
    ...
    async elementTriggered(evt) {
        ...
        try {
            ...
            console.log(`aurora-action: '${this.attribute.value}' eval error >`, e.message);
        }
        delete this._active;
    }
    ...
    get fn() {
        if (!this._fn) {
            try {
                ...
                const fn       = ev.buildFN(this.attribute.value, [...this.elementAndAttributeParamNames(), '$event']);

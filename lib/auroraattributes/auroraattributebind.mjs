        ...
        try {
            ...
            if (element.type?.startsWith('select-') && prop === 'innerHTML') {
                ...
                // if (!(prop === 'src' && result == undefined)) { // don't set src properties of attributes to undefined
                    ...
                    if (isUFD(result)) result = await result.getObjectURL();   // objects can't be set in attributes, get the object url for the unified file descriptor
                    const value = element[prop];
                    if (value != result) element[prop] = result;
                // }
            } else {
                // if (!(prop === 'src' && result == undefined)) {

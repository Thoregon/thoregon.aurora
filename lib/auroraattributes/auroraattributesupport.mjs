...
export default (base) => class AuroraAttributeSupport extends (base || Object) {
    ...
    async parentViewModelHasBeenAttached(viewModel, retry = 3) {
        if (!this.useParentViewModel()) return;
        if (!this.container) {
            if (retry === 0) return;
            setTimeout(() => this.parentViewModelHasBeenAttached(viewModel, --retry), 100);
            return;
        }
    ...
    getAuroraAttributes() {
        ...
        if (!auroraattributes) auroraattributes = this.auroraAttributes =  [];
    ...
    get auroraAttributes() {
        let auroraattributes = this._auroraAttributes;
        if (!auroraattributes) auroraattributes = this._auroraAttributes =  [];
        return auroraattributes;
    }

    set auroraAttributes(auroraattributes) {
        this._auroraAttributes = auroraattributes;
    }

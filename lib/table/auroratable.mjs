...
export default class AuroraTable extends AuroraFormElement {
    ...
    async existsConnect() {
        ...
        let function_columns = propertiesValues[':columns'];
        let function_query   = propertiesValues[':query'];

        let vm      = this.viewModel || this.parentViewModel();
        let columns = this.getColumnDefinition( vm, function_columns );
        let query   = vm[function_query]();

        this.container.innerHTML = this.renderTable( columns, query);
    ...
    renderTable( columns, query ) {

        let display = '';

        display += this.renderTableHeader( columns, query );
        display += this.renderTableBody( columns, query );

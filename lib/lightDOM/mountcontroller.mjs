...
export default class MountController {
    ...
    constructor({ host, slotname, style } = {}) {
    ...
    static forHost(host, slotname, style) {
        const controller = new this({ host, slotname, style });
...
//            div.setAttribute('style', styleString);
        ...
        hosts.forEach(host => {
            const slot = host.querySelector(`[slot="${slotname}"]`);
            if (!slot) this.appendTemplate(host, this.createSlot(slotname));
        });
    }
}
